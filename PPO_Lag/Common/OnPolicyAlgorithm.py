...
from .Policises import SafeActorCriticPolicy
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                actions, values, log_probs, c_values= self.policy(obs_tensor)
            ...
            for idx, done in enumerate(dones):
                if (
                    ...
                    with th.no_grad():
                        ...
                        #terminal_value ,terminal_c_value= self.policy.predict_values(terminal_obs)[0]  # type: ignore[arg-type]
                        terminal_value, terminal_c_value = [v[0] for v in self.policy.predict_values(terminal_obs)]
        ...
        with th.no_grad():
            ...
            values, c_values = self.policy.predict_values(obs_as_tensor(new_obs, self.device))  # type: ignore[arg-type]
            #values = self.policy.predict_values(obs_as_tensor(new_obs, self.device))  # type: ignore[arg-type]

        rollout_buffer.compute_returns_and_advantage(last_values=values, dones=dones, last_c_values=c_values)
